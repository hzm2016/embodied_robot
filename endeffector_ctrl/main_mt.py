...
class VoiceSubscriber(threading.Thread):  
    ...
    def handle_command(self, cmd):   
        # 把语音转换为已有控制路径    
        print("Received voice command:", cmd)
        if not cmd:   
            return   
...
if __name__ == '__main__':
    ...
    publisher_endo = Publisher(key="endo")  
    # Create a thread for publishing messages
    publisher_thread_endo = threading.Thread(target=publisher_endo.publish_messages)
    # Start the publisher thread
    publisher_thread_endo.start()  
    ...
    try:   
       while True:   
        ...
        publisher_endo.update_positions(endo_start, endo_end, rcm_point)
