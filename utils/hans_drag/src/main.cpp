...
//     // 在当前姿态下采样 N 次并建立偏置（会阻塞调用线程 N*周期）
...
//         // 为了得到有效的重力补偿，采样期间需要更新 cur_rot/cur_pos
...
//     // 调试打印可按需保留/注释
//     // std::cout<<"Force after: "<<real_f.transpose()<<","<<real_t.transpose()<<std::endl;
...
//         const bool enable_start_bias = true; // 改为 false 可完全禁用该功能
//         const int  start_bias_samples = 100; // 采样次数（约 samples*12ms）
...
//                 drag.slow_scale = std::max(0.2, drag.slow_scale); // 最低20%
//             }

//             // if (sigma_min < sigma_hard_stop) {
//             //     std::cout << "[Singularity] sigma_min=" << sigma_min << " -> HARD STOP\n";
//             //     // 冻结在当前位置（保持当前关节）
...
//             //     // 清零内部速度，避免解除后跳变
//             //     drag.cur_velo.setZero(); drag.next_velo.setZero();
//             //     drag.cur_ori_velo.setZero(); drag.next_ori_velo.setZero();
//             //     drag.delta_pos.setZero();
//             //     drag.delta_theta.setZero();

//             //     std::this_thread::sleep_until(next_tick);
//             //     continue;
//             // } else if (sigma_min < sigma_slow) {
//             //     // 提高阻尼
//             //     drag.pos_d = 70.0f;
//             //     drag.ori_d = 30.0f;
//             // } else {
