...
class RealtimePoseCollisionSystem:
    ...
    def __init__(self, robot_base_position=None, capsule_radius=0.1, cube_size=0.5, 
                 enable_prediction=False, use_rgbd=False, Realrobot=False):
        # 基础组件
    ...
    def initialize(self):
        ...
        # 初始化摄像头
        self.camera_manager.initialize(use_rgbd=self.use_rgbd)
        
        print("System initialized successfully!")
        print("\nFeatures enabled:")
        print("  ✓ Real-time 2D pose detection")
        print("  ✓ 3D pose estimation")
        print("  ✓ Collision detection")
        print("  ✓ Robot control")
        print(f"  {'✓' if self.enable_prediction else '✗'} Motion prediction")
        print(f"  Camera: {'RGBD (L515)' if self.use_rgbd else 'RGB'}")
    ...
    def run(self):
        ...
        # self.robot.set_joint_angles(theta)
        ...
        while True:
            if not paused:
                ...
                frame_count += 1
                # 获取2D关键点
                input_2D_no, input_2D_sc = self.pose_estimator.get_2d_keypoints(frame)
                # 估计3D姿态
                if self.use_rgbd:
                    # RGBD模式：使用2D关键点+深度信息直接计算3D位置
                    ...
                    # RGB模式：使用传统的3D姿态估计
                    pose_3d_17, confidence = self.pose_estimator.estimate_3d_pose(
                        input_2D_no, input_2D_sc, frame.shape)
                ...
                try:
                    ...
                    if self.enable_prediction and pose_3d_10 is not None:
                        ...
                        if predicted_motion is not None:
                            ...
                            if self.use_rgbd:
                                ...
                                offset2 = np.array([-0.4, 0.3, 0.4]) - predicted_motion[0, 1, :]
                                predicted_motion += offset2
                    ...
                    if self.use_rgbd:
                        ...
                        offset = np.array([-0.4, 0.3, 0.4]) - pose_3d_10[1] if pose_3d_10[1] is not None else np.array([0, 0, 0])
                        pose_3d_10 += offset
                ...
                # 绘制2D姿态
                frame_with_2d = self.visualizer.draw_2d_pose(frame, input_2D_no, input_2D_sc)
                # 3D姿态，消耗很大
                ...
                if pose_3d_10 is not None:
                    self.robot.set_human_model(pose_3d_10)
                if self.enable_prediction and pose_3d_pred is not None:
                    self.robot.set_predict_human_model(pose_3d_pred)
                self.robot.step_simulation()
                '''
                # 在2D图像上显示状态信息
                self._draw_status_info(frame_with_2d, collision_result, 
                                     predicted_motion, future_collision_risk, frame_count)
                
                # 显示2D摄像头画面
                window_title = 'Enhanced Real-time System' if self.enable_prediction else '2D Real-time Camera + Pose Detection'
                cv2.imshow(window_title, frame_with_2d)
