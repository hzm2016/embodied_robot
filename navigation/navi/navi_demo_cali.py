...
HEAD_POINT_COUNT = 100000        # 点云模式的采样点数（仅当 HEAD_DISPLAY_MODE="pointcloud" 时生效）
...
if __name__ == "__main__":
    ...
    if HEAD_DISPLAY_MODE.lower() == "pointcloud":
        head_display = make_head_pointcloud_from_mesh(mesh_head_tri, color=(1.0, 0.85, 0.4), n_points=HEAD_POINT_COUNT)
    else:
        head_display = make_head_wireframe_from_mesh(mesh_head_tri, color=(1.0, 0.8, 0.2))

    mesh_brain = o3d.io.read_triangle_mesh("BRAIN2_hole.stl")
    mesh_brain.compute_vertex_normals()
    mesh_brain.paint_uniform_color([0.7, 0.7, 0.7])  # 保持不透明与原样
    ...
    # 线宽在不同平台可能不生效
    # if hasattr(opt3, "line_width"):
    #     opt3.line_width = 2.0

    # 分别添加 head(线框/点云) 与 brain(不透明)
    ...
    while True:
        ...
        # 更新/显示相机坐标系（与第一视角相机外参一致）——两个窗口都显示（默认关闭）
