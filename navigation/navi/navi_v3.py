...
def cal_endo2ee(pos):
    '''
    pos-[分别代表内镜伸缩电机(0,负)、竖直基座电机(1-2,负)和水平电机(3-4,正),单位mm]
    '''
    ## 定长(mm)，0~4对应坐标系序号
    Ly0 = 60  # Y0方向基座长度
    ...
    Lx2 = 67  # X2方向34杆长度
    Ly2 = 59  # Y2方向34杆间距
    Lz2 = 102  # Z2方向平台到坐标系3距离
    Ly4 = 250  # Y4方向到内镜头初始距离
    L = [Ly4 + pos[0], Lz0 + pos[1], Lz0 + pos[2], Lx2 - pos[3], pos[4]]
    psi12 = np.arctan2(L[1] - L[2], Ly0)
    psi34 = np.arctan2(L[3] - L[4] , Ly2)
    T01 = pose_to_transformation_matrix([0,0,L[1],0,0,0])
    T12 = pose_to_transformation_matrix([0,0,0,-psi12,0,0])
    T23 = pose_to_transformation_matrix([0,Lz2, L[3],0, 0, 0])
    T34 = pose_to_transformation_matrix([0,0,0 ,0, 0, psi34])
    T04 = T01 @ T12 @ T23 @ T34
    return T04
...
if __name__ == "__main__":
    ...
    model2camera[:3,3] = model2camera[:3,3]*1000
    camera2model = np.linalg.inv(model2camera) # 可变?
    ee2camera = pose_to_transformation_matrix([-80,-40,-40,np.pi/2,0,0]) # 不可变，以远离手术台和相机的底角为坐标轴
    camera2ee = np.linalg.inv(ee2camera)
    ee2model = camera2model @ ee2camera
    endo_pos = [-20,-55/2,-55/2,55/2,55/2]
    # endo2ee = pose_to_transformation_matrix([0,220,200,-np.pi/2,0,0]) # 可变
    endo2ee = cal_endo2ee(endo_pos)
    ...
    # 绘制endoscopy，原点为内镜，因此往z轴负方向延申内镜长度
    ...
    while True:
        ...
        v_joint = robot_new_Jacob.GetEnd2JointVel(v_e, endo_pos[1:]) # 获得各关节移动速度
        v_joint = v_joint*1000
        endo_pos[1:4] = [endo_pos[1]+v_joint[0].item(),endo_pos[2]+v_joint[1].item(),endo_pos[3]+v_joint[2].item(),endo_pos[4]+v_joint[3].item()]
        v_s = np.matrix([0.,0.,0.,0.]).T
