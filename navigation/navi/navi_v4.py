...
if __name__ == "__main__":
    ...
    model2camera = np.load("/home/mingcong/projects/p9/regesiteration/data/trans_matrix_base2model.npy")
    model2camera[:3,3] = model2camera[:3,3]*1000
    camera2model = np.linalg.inv(model2camera) # 可变?
    ee2camera = pose_to_transformation_matrix([-80,60,20,np.pi/2,0,0]) # 不可变，以远离手术台和相机的底角为坐标轴
    camera2ee = np.linalg.inv(ee2camera)
    
    base2model = np.load("/home/mingcong/projects/p9/regesiteration/data/trans_matrix_base2model.npy")
    ee2model = np.load("/home/mingcong/projects/p9/regesiteration/data/trans_matrix_ee2model.npy")
    ...
    print("####################3")
    print(joint_rad)
    # joint_rad = [2.053589304481568, 0.2028770722518209, -0.44338344317663947, -1.0702009907378829, -1.150503589622142, 1.3167985607521617, -2.618761822862372]
