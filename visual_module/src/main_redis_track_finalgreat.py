...
def main() -> None:
	...
	gripper_model_path = "./checkpoint/tool_best.pt"
	...
	model = DetectModel(current_model_path, conf_threshold=tag_conf)
	scope_controller = ScopeControl(track_goal="blue tag", threshold=tag_conf)
	...
	try:
		while not exit_event.is_set():
			...
			if track_flag is True and find_flag is False:
				...
				vjoint = GetEnd2JointVel(vend, theta, din)*0.8
			...
			if key in (27, ord("q")):
				...
				scope_controller.change_goal("gripper" if gripper_flag else "one")
				if gripper_flag:
					model.change_conf_threshold(gripper_conf)
					scope_controller.change_threshold(gripper_conf)
				else:
					model.change_conf_threshold(tag_conf)
					scope_controller.change_threshold(tag_conf)
	finally:
		exit_event.set()
		cap.release()
