import numpy as np
from .Kinematics import GetFKinematics, GetReduJointVel, GetIKinematics
...
class ControlSystem:
    def __init__(self):
        ...
        self.index = 4  # 冗余关节索引
        self.dt = 0.01
        self.vavoid_ori = np.array([0., 0., 0., 0., 0., -0.2])
    ...
    def _set_Td(self, theta):
        Td, _ = GetFKinematics(theta)
        self.Td = Td

    def _check_Td(self, theta):
        if self.Td is not None:
            theta = GetIKinematics(self.Td, theta)
        return theta
    ...
    def update(self, collision_detector, theta):
        ...
        self.theta = self._check_Td(self.theta)  
        return self.theta  
